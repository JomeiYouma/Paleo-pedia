...
const ArticleForm = ({ initial, onCancel, onSubmit, busy, submitLabel }) => {
    const { t } = useTranslation();
    const resolvedSubmitLabel = submitLabel || t('adminPress.save', 'Enregistrer');
    ...
    const handleThumb = async (file) => {
        ...
        try {
            ...
            setUploadError(e.message || t('adminPress.uploadFailed', "Échec de l'upload"));
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div>
                <label style={labelStyle}>{t('adminPress.titleLabel', 'Titre *')}</label>
                <input value={title} onChange={e => setTitle(e.target.value)} required style={inputStyle}
                    placeholder={t('adminPress.titlePlaceholder', "Cédric Carles, le designer fédérateur d'énergies")} />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px' }}>
                <div>
                    <label style={labelStyle}>{t('adminPress.sourceLabel', 'Source / média')}</label>
                    <input value={source} onChange={e => setSource(e.target.value)} style={inputStyle}
                        placeholder={t('adminPress.sourcePlaceholder', 'Le Monde, RTBF, Ouest France…')} />
                </div>
                <div>
                    <label style={labelStyle}>{t('adminPress.dateLabel', 'Date de publication')}</label>
                    <input type="date" value={date} onChange={e => setDate(e.target.value)} style={inputStyle} />
                </div>
            </div>

            <div>
                <label style={labelStyle}>{t('adminPress.urlLabel', "URL de l'article")}</label>
                <input value={url} onChange={e => setUrl(e.target.value)} style={inputStyle}
                    placeholder={t('adminPress.urlPlaceholder', 'https://lemonde.fr/article-…')} />
            </div>

            <div>
                <label style={labelStyle}>{t('adminPress.excerptLabel', 'Accroche / résumé (optionnel)')}</label>
                <textarea value={excerpt} onChange={e => setExcerpt(e.target.value)} rows={2}
                    style={{ ...inputStyle, resize: 'vertical' }}
                    placeholder={t('adminPress.excerptPlaceholder', "Une phrase qui résume l'article — affichée sous le titre.")} />
            </div>

            {/* Vignette */}
            <div>
                <label style={labelStyle}>{t('adminPress.thumbnailLabel', 'Vignette')}</label>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    <label style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 12px', border: '1px dashed var(--color-border-strong)', borderRadius: 'var(--radius-md)', cursor: uploading ? 'wait' : 'pointer', fontSize: '0.85rem', color: 'var(--color-text-muted)', background: 'var(--color-surface-2)' }}>
                        <Upload size={14} />
                        {uploading ? t('adminPress.uploading', 'Upload en cours…') : (thumbnailPath ? t('adminPress.replaceThumbnail', 'Remplacer la vignette') : t('adminPress.chooseThumbnail', 'Choisir une vignette…'))}
                        <input type="file" accept="image/*" disabled={uploading} style={{ display: 'none' }} onChange={e => handleThumb(e.target.files?.[0])} />
                    </label>
                    {thumbnailPath && (
                        <button type="button" onClick={() => setThumbnailPath('')} style={ghostBtnStyle} title={t('adminPress.removeThumbnail', 'Retirer la vignette')}>
                            <X size={14} />
                        </button>
                    )}
                </div>
            ...
            <fieldset style={{
                ...
                <legend style={{ fontFamily: 'var(--font-heading)', fontSize: '0.78rem', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--color-text-muted)', padding: '0 8px' }}>
                    {t('adminPress.englishVersion', 'Version anglaise')}
                </legend>
                ...
                <div>
                    <label style={labelStyle}>{t('adminPress.titleEnLabel', 'Title (EN)')}</label>
                    <input value={titleEn} onChange={e => setTitleEn(e.target.value)} style={inputStyle} />
                </div>
                <div style={{ marginTop: '10px' }}>
                    <label style={labelStyle}>{t('adminPress.excerptEnLabel', 'Excerpt (EN)')}</label>
            ...
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', color: 'var(--color-text-muted)', marginTop: '4px' }}>
                ...
                {t('adminPress.publishedCheckbox', 'Article publié (visible sur la page presse)')}
            </label>

            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '4px' }}>
                {onCancel && (
                    <button type="button" onClick={onCancel} disabled={busy} style={ghostBtnStyle}>
                        {t('adminPress.cancel', 'Annuler')}
                    </button>
                )}
                <button type="submit" disabled={busy || !title.trim()} style={{
                    ...
                    <Save size={14} /> {busy ? t('adminPress.sending', 'Envoi…') : resolvedSubmitLabel}
                </button>
            </div>
        </form>
    );
...
const AdminPress = () => {
    ...
    const load = async () => {
        ...
        try {
            ...
            showToast('error', e.message || t('adminPress.loadError', 'Erreur de chargement'));
        } finally {
            setLoading(false);
        }
    };
    ...
    if (!isSuperadmin) {
        return (
            <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--color-text-subtle)' }}>
                {t('adminPress.accessRestricted', 'Accès réservé aux superadmins.')}
            </div>
        );
    }
    ...
    const handleCreate = async (data) => {
        ...
        try {
            ...
            setShowForm(false);
            showToast('success', t('adminPress.createdToast', { title: created.title, defaultValue: '« {{title}} » ajouté' }));
        } catch (e) {
            showToast('error', e.message || t('adminPress.createError', 'Erreur création'));
        } finally {
            setCreating(false);
        }
    };
    ...
    const handleUpdate = async (id, data) => {
        ...
        try {
            ...
            setEditingId(null);
            showToast('success', t('adminPress.updatedToast', { title: updated.title, defaultValue: '« {{title}} » mis à jour' }));
        } catch (e) {
            showToast('error', e.message || t('adminPress.updateError', 'Erreur mise à jour'));
        } finally {
            setBusyId(null);
        }
    };
    ...
    const handleTogglePublish = async (a) => {
        ...
        try {
            ...
            showToast('success', updated.is_published ? t('adminPress.articlePublished', 'Article publié') : t('adminPress.articleHidden', 'Article masqué'));
        } catch (e) {
            showToast('error', e.message || t('adminPress.genericError', 'Erreur'));
        } finally {
            setBusyId(null);
        }
    };
    ...
    const handleDelete = async (a) => {
        if (!confirm(t('adminPress.deleteConfirm', { title: a.title, defaultValue: 'Supprimer définitivement « {{title}} » ?' }))) return;
        setBusyId(a.id);
        try {
            await api.pressArticles.delete(a.id);
            setArticles(prev => prev.filter(x => x.id !== a.id));
            showToast('success', t('adminPress.deletedToast', { title: a.title, defaultValue: '« {{title}} » supprimé' }));
        } catch (e) {
            showToast('error', e.message || t('adminPress.deleteError', 'Erreur suppression'));
        } finally {
            setBusyId(null);
        }
    };
    ...
    return (
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '28px 24px 80px' }}>
            ...
            <AdminPageHeader icon={Newspaper} title={t('adminPress.pageTitle', 'Articles de presse')} />

            <ExplainerBox title={t('adminPress.explainerTitle', 'À quoi sert cette page ?')}>
                {t('adminPress.explainerP1', 'Gérer les articles de presse affichés sur la ')}
                <strong>{t('adminPress.explainerStrong', 'page publique « Presse »')}</strong>
                {t('adminPress.explainerP2', '. Les articles sont triés automatiquement par date de publication décroissante (les plus récents en premier). Vous pouvez masquer un article temporairement sans le supprimer en décochant « Article publié ».')}
            </ExplainerBox>
            ...
            {!showForm && (
                ...
                >
                    <Plus size={14} /> {t('adminPress.addArticle', 'Ajouter un article')}
                </button>
            )}
            {showForm && (
                <AdminSection>
                    <p style={{ margin: '0 0 14px', fontWeight: '800', fontSize: '0.82rem', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--color-primary)', fontFamily: 'var(--font-heading)' }}>
                        {t('adminPress.newArticle', 'Nouvel article')}
                    </p>
                    <ArticleForm
                        onCancel={() => setShowForm(false)}
                        onSubmit={handleCreate}
                        busy={creating}
                        submitLabel={t('adminPress.add', 'Ajouter')}
                    />
                </AdminSection>
            )}
            ...
            <AdminSection>
                {loading ? (
                    <p style={{ textAlign: 'center', color: 'var(--color-text-subtle)', padding: '40px 0' }}>{t('adminPress.loading', 'Chargement…')}</p>
                ) : articles.length === 0 ? (
                    <p style={{ textAlign: 'center', color: 'var(--color-text-subtle)', padding: '40px 0', fontSize: '0.9rem' }}>
                        {t('adminPress.noArticles', 'Aucun article. Ajoutez le premier ci-dessus.')}
                    </p>
                ) : (
                                ...
                                return (
                                    <div key={a.id} style={{ border: '2px solid var(--color-accent)', borderRadius: 'var(--radius-md)', padding: '16px', background: 'var(--color-surface)' }}>
                                        <p style={{ margin: '0 0 12px', fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.5px', fontFamily: 'var(--font-heading)', fontWeight: '700', color: 'var(--color-text-muted)' }}>
                                            {t('adminPress.editingHeading', { title: a.title, defaultValue: 'Édition de « {{title}} »' })}
                                        </p>
                                        <ArticleForm
                                            initial={a}
                                ...
                                <div key={a.id} style={{
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: '0.74rem', color: 'var(--color-text-subtle)', textTransform: 'uppercase', letterSpacing: '0.5px', fontFamily: 'var(--font-heading)', fontWeight: '700' }}>
                                            ...
                                            {!a.is_published && <span style={{ marginLeft: '8px', color: 'var(--color-warning)' }}>{t('adminPress.hiddenBadge', '· masqué')}</span>}
                                        </div>
                                    ...
                                    <button
                                        ...
                                        title={a.is_published ? t('adminPress.hide', 'Masquer') : t('adminPress.publish', 'Publier')}
                                    >
                                    ...
                                    <button
                                        ...
                                        title={t('adminPress.edit', 'Modifier')}
                                    >
                                        <Pencil size={13} />
                                    </button>
                                    <button
                                        ...
                                        title={t('adminPress.delete', 'Supprimer')}
                                    >
                                        <Trash2 size={13} />
                                    </button>
                                </div>
